...
class _CartScreenState extends State<CartScreen> {
  ...
  Future<void> _urunuSil(int id) async {
    ...
    ScaffoldMessenger.of(context).showSnackBar(
      SnackBar(
        content: const Text("Ürün sepetten çıkarıldı.", style: TextStyle(color: kBookPaper)),
      ...
      child: Column(
        ...
        children: [
          Container(
            ...
            child: Icon(Icons.shopping_basket_outlined, size: 60, color: kOliveGreen.withOpacity(0.5)),
          ),
          ...
          children: [
            Row(
              ...
              children: [
                const Text(
                  "Toplam Tutar", 
                  style: TextStyle(fontSize: 16, fontWeight: FontWeight.w600, color: kOliveGreen)
                ),
                Text(
                  "${_toplamTutar.toStringAsFixed(2)} ₺", 
                  style: const TextStyle(fontSize: 24, fontWeight: FontWeight.bold, color: kDarkGreen)
                ),
              ],
            ),
            const SizedBox(height: 20),
